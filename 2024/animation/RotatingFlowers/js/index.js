...
fs.sort(function(a,b) { return parseFloat(b.size) - parseFloat(a.size) } );
...
function draw() {
    setTimeout(function() {
        ...
        for (var i = 0; i < num_flowers; i++){
            draw_flower(sf*fs[i].size/10* 0.8, fs[i].petals*2, 0,fs[i].rot/15,fs[i].c,fs[i].x,fs[i].y);
        }
...
function add_flower(_x,_y){
    fs.push({size:rand(1,3),petals:rand(4,12),rot:rand(-15,15),c:[rand(100,255),rand(100,255),rand(100,255)],x:_x,y:_y});
    num_flowers++;
}
